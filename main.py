...
app = FastAPI(title="AI Code Review Agent - Simple Version")
...
def process_pr_analysis(task_id: str, repo_url: str, pr_number: int, github_token: str = None):
    try:
        ...
        tasks_store[task_id]['results'] = results
...
async def root():
    return {
        "message": "AI Code Review Agent API - Simple Version (OpenAI)",
        "endpoints": {
            "POST /analyze-pr": "Submit a PR for analysis",
            "GET /status/{task_id}": "Check task status",
            "GET /results/{task_id}": "Get analysis results"
        },
        "note": "Tasks are stored in memory. They will be lost on server restart."
    }